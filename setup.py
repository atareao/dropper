...
DEST_DIR = '/opt/extras.ubuntu.com/dropper'
DATA_FILES = [
	(os.path.join(DEST_DIR,'bin'),glob.glob('bin/*')),
	(os.path.join(DEST_DIR,'share','dropper'),glob.glob('src/*')),
	(os.path.join(DEST_DIR,'share','dropper'),glob.glob('data/icons/*')),
	('/usr/share/applications',['data/extras-dropper.desktop'])
	]
DESCRIPTION = 'A tool to get colors'
LONG_DESCRIPTION = 'A tool to get colors'
MAIN_DIR = os.getcwd()
...
class install_data(_install_data):
	def run(self):
		for lang in os.listdir('build/locale-langpack/'):
			lang_dir = os.path.join(DEST_DIR, 'share', 'locale-langpack', lang, 'LC_MESSAGES')
